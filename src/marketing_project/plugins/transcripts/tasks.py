...
from marketing_project.core.parsers import clean_text, parse_transcript
...
def extract_transcript_metadata(transcript: TranscriptContext) -> Dict[str, Any]:
    ...
    if transcript.timestamps or parsed_data.get("timestamps"):
        metadata["timestamp_count"] = len(
            transcript.timestamps or parsed_data.get("timestamps", {})
        )
...
def enhance_transcript_with_ocr(
    transcript: TranscriptContext, image_urls: List[str] = None
) -> Dict[str, Any]:
    """
    ...
    enhanced_data = enhance_content_with_ocr(
        transcript.content, "transcript", image_urls=image_urls
    )
...
def route_transcript_processing(
    ...
    if agent_name in available_agents and available_agents[agent_name]:
        ...
        logger.warning(
            f"No agent available for {processing_type} transcript, using general processing"
        )

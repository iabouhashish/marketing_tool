...
from marketing_project.core.parsers import clean_text, parse_blog_post
...
def analyze_blog_post_type(blog_post: BlogPostContext) -> str:
    ...
    if any(
        ...
        keyword in title_lower for keyword in ["analysis", "deep dive", "explanation"]
    ):
        return "analysis"
...
def extract_blog_post_metadata(blog_post: BlogPostContext) -> Dict[str, Any]:
    ...
    metadata = {
        ...
        "category": blog_post.category
        or parsed_data.get("metadata", {}).get("category", ""),
        ...
        "has_author": bool(
            blog_post.author or parsed_data.get("metadata", {}).get("author")
        ),
        "has_category": bool(
            blog_post.category or parsed_data.get("metadata", {}).get("category")
        ),
...
def enhance_blog_post_with_ocr(
    blog_post: BlogPostContext, image_urls: List[str] = None
) -> Dict[str, Any]:
    """
    ...
    enhanced_data = enhance_content_with_ocr(
        blog_post.content, "blog_post", image_urls=image_urls
    )
...
def route_blog_post_processing(
    ...
    if agent_name in available_agents and available_agents[agent_name]:
        ...
        logger.warning(
            f"No agent available for {processing_type} blog post, using general processing"
        )

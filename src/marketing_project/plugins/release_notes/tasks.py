...
from marketing_project.core.parsers import clean_text, parse_release_notes
...
def extract_release_metadata(release_notes: ReleaseNotesContext) -> Dict[str, Any]:
    ...
    metadata = {
        ...
        "breaking_changes_count": len(
            release_notes.breaking_changes or parsed_data.get("breaking_changes", [])
        ),
        "features_count": len(
            release_notes.features or parsed_data.get("features", [])
        ),
        "bug_fixes_count": len(
            release_notes.bug_fixes or parsed_data.get("bug_fixes", [])
        ),
        ...
        "has_breaking_changes": len(
            release_notes.breaking_changes or parsed_data.get("breaking_changes", [])
        )
        > 0,
        "has_new_features": len(
            release_notes.features or parsed_data.get("features", [])
        )
        > 0,
...
def enhance_release_notes_with_ocr(
    release_notes: ReleaseNotesContext, image_urls: List[str] = None
) -> Dict[str, Any]:
    """
    ...
    enhanced_data = enhance_content_with_ocr(
        release_notes.content, "release_notes", image_urls=image_urls
    )
...
def route_release_processing(
    ...
    if agent_name in available_agents and available_agents[agent_name]:
        ...
        logger.warning(
            f"No agent available for {processing_type} release, using general processing"
        )

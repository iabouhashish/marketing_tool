...
def process_content_images(content: str, image_urls: List[str] = None, 
                         image_data: List[bytes] = None) -> Dict[str, Any]:
    """
    ...
    if ocr_results["total_ocr_text"]:
        ocr_results["enhanced_content"] = content + "\n\n--- OCR Extracted Text ---" + ocr_results["total_ocr_text"]
    
    return ocr_results

def enhance_content_with_ocr(content: str, content_type: str, 
...
def extract_images_from_content(content: str) -> List[str]:
    ...
    for url in urls:
        try:
            parsed = urlparse(url)
            if parsed.path.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.svg')):

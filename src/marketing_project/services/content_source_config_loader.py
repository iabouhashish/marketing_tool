...
from marketing_project.core.content_sources import (
    FileSourceConfig, APISourceConfig, DatabaseSourceConfig, 
    WebScrapingSourceConfig, WebhookSourceConfig, RSSSourceConfig,
    SocialMediaSourceConfig, ContentSourceType
)
...
class ContentSourceConfigLoader:
    ...
    def _convert_yaml_to_config(self, yaml_config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def create_source_configs(self) -> List[Any]:
        ...
        for config in configs:
            try:
                ...
                logger.error(f"Failed to create source config for {config.get('name')}: {e}")
                continue

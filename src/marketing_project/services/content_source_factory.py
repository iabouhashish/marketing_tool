...
from marketing_project.core.content_sources import (
    ContentSource, ContentSourceManager as BaseContentSourceManager,
    ContentSourceResult, SourceConfig, ContentSourceType
)
...
from marketing_project.services.file_source import (
    FileContentSource, DirectoryWatcherSource, UploadedFileSource
)
from marketing_project.services.api_source import (
    APIContentSource, WebhookContentSource, RSSContentSource
)
...
from marketing_project.services.web_scraping_source import (
    WebScrapingContentSource, SeleniumScrapingSource, BeautifulSoupScrapingSource
)
...
class ContentSourceFactory:
    ...
    @staticmethod
    def create_uploaded_file_source(name: str, upload_directory: str = "uploads") -> UploadedFileSource:
...
class ContentSourceManager(BaseContentSourceManager):
    ...
    async def add_multiple_sources(self, configs: List[SourceConfig]) -> Dict[str, bool]:
    ...
    async def fetch_content_with_cache(self, limit_per_source: Optional[int] = None, 
                                     use_cache: bool = True) -> List[ContentSourceResult]:
    ...
    async def fetch_content_as_models(self, limit_per_source: Optional[int] = None) -> List[ContentContext]:
    ...
    async def get_content_by_type(self, content_type: str, 
    ...
    async def get_content_models_by_type(self, content_type: str, 
                                       limit_per_source: Optional[int] = None) -> List[ContentContext]:
    ...
    def _model_matches_type(self, content_model: ContentContext, content_type: str) -> bool:
    ...
    def _content_matches_type(self, content_item: Dict[str, Any], content_type: str) -> bool:
    ...
    async def search_content(self, query: str, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    async def search_content_models(self, query: str, limit: Optional[int] = None) -> List[ContentContext]:
        ...
        for content_model in content_models:
            # Search in title, content, and snippet
            searchable_text = f"{content_model.title} {content_model.content} {content_model.snippet}"
            if query_lower in searchable_text.lower():

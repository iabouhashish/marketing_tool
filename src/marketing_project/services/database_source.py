...
from marketing_project.core.content_sources import (
    DatabaseSourceConfig, ContentSource, ContentSourceResult, ContentSourceStatus
)
...
class SQLContentSource(DatabaseContentSource):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if connection_string.startswith("sqlite"):
                import aiosqlite
                self.connection = await aiosqlite.connect(connection_string.replace("sqlite://", ""))
            ...
            logger.error(f"Failed to initialize SQL database source {self.config.name}: {e}")
    ...
    async def fetch_content(self, limit: Optional[int] = None) -> ContentSourceResult:
        ...
        try:
            ...
            logger.error(f"Failed to fetch content from SQL database {self.config.name}: {e}")
            return ContentSourceResult(
                source_name=self.config.name,
                content_items=[],
                total_count=0,
    ...
    def _convert_row_to_content_item(self, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            content_item = {
                ...
                "title": row.get("title", row.get("name", row.get("subject", "Untitled"))),
                "content": row.get("content", row.get("body", row.get("text", row.get("description", "")))),
                "snippet": row.get("snippet", row.get("excerpt", row.get("summary", ""))),
                "created_at": row.get("created_at", row.get("date", row.get("timestamp", datetime.now().isoformat()))),
            ...
            if "speakers" in row or "participants" in row:
                ...
                content_item.update({
                    ...
                    "word_count": row.get("word_count", len(content_item["content"].split()))
                })
        ...
        try:
            ...
            if self.config.where_clause:
                ...
                try:
                    ...
                    logger.warning(f"Invalid where clause JSON: {self.config.where_clause}")
            ...
            logger.error(f"Failed to fetch content from MongoDB {self.config.name}: {e}")
            return ContentSourceResult(
                source_name=self.config.name,
                content_items=[],
                total_count=0,
    ...
    def _convert_document_to_content_item(self, doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            content_item = {
                ...
                "title": doc.get("title", doc.get("name", doc.get("subject", "Untitled"))),
                "content": doc.get("content", doc.get("body", doc.get("text", doc.get("description", "")))),
                "snippet": doc.get("snippet", doc.get("excerpt", doc.get("summary", ""))),
                "created_at": doc.get("created_at", doc.get("date", doc.get("timestamp", datetime.now().isoformat()))),
            ...
            if "speakers" in doc or "participants" in doc:
                ...
                content_item.update({
                    ...
                    "word_count": doc.get("word_count", len(content_item["content"].split()))
                })
...
class RedisContentSource(DatabaseContentSource):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if connection_string.startswith("redis://"):
                ...
                host, port = connection_string.split(":") if ":" in connection_string else (connection_string, 6379)
            ...
            for key in keys:
                try:
                    ...
                    if value:
                        ...
                        content_item = self._convert_redis_value_to_content_item(data, key)
                        if content_item:
                            content_items.append(content_item)
    ...
    def _convert_redis_value_to_content_item(self, data: Dict[str, Any], key: str) -> Dict[str, Any]:
        """Convert Redis value to content item."""
        try:
            content_item = {
                "id": str(data.get("id", key)),
                "title": data.get("title", data.get("name", key)),
                "content": data.get("content", data.get("body", data.get("text", str(data)))),

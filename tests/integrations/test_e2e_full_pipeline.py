...
def test_prompts_dir(tmp_path):
    """Set up test prompts directory with real templates."""
    ...
    if prompts_source_dir.exists():
        ...
        logger.info(f"Copied {len(list(prompts_source_dir.glob('*.j2')))} prompt templates to test directory")
    else:
        # Fallback: create minimal templates if source doesn't exist
        required_agents = [
            "transcripts_agent", "blog_agent", "releasenotes_agent", 
            "marketing_orchestrator_agent", "content_pipeline_agent",
            "seo_keywords_agent", "marketing_brief_agent", "article_generation_agent",
            "seo_optimization_agent", "internal_docs_agent", "content_formatting_agent",
            "design_kit_agent"
        ]
        for name in required_agents:
            (prompts_test_dir / f"{name}_instructions.j2").write_text("You are a helpful AI agent.")
            (prompts_test_dir / f"{name}_description.j2").write_text("AI agent for content processing.")
        logger.warning("Created minimal prompt templates - using fallback")
    
    return str(prompts_test_dir)
...
class TestE2EFullPipeline:
    ...
    async def test_content_source_loading(self, test_content_files):
        ...
        file_sources = [name for name, source in content_manager.sources.items() 
                       if hasattr(source, 'config') and source.config.source_type.value == 'file']
    ...
    @pytest.mark.asyncio
    async def test_original_3_agent_pipeline(self, test_prompts_dir, test_content_files):
        ...
        result = await run_marketing_project_pipeline(prompts_dir=test_prompts_dir, lang="en")
        ...
        assert "processed_content" in result, "Pipeline result missing processed_content"
        
        # Verify agents were created
        agents = result["agents"]
        expected_agents = ["transcripts_agent", "blog_agent", "releasenotes_agent", "orchestrator_agent"]
        ...
        assert len(processed_content) > 0, "No content was processed"
        
        logger.info(f"Original pipeline processed {len(processed_content)} content items")
        
        # Cleanup
        await result["content_manager"].cleanup()
    
    @pytest.mark.asyncio
    async def test_content_analysis_pipeline_8_steps(self, test_prompts_dir, test_content_files):
        ...
        result = await run_content_analysis_pipeline(prompts_dir=test_prompts_dir, lang="en")
        
        # Verify pipeline components
        assert "content_pipeline_agent" in result, "Pipeline result missing content_pipeline_agent"
        assert "content_manager" in result, "Pipeline result missing content_manager"
        assert "processed_content" in result, "Pipeline result missing processed_content"
        ...
        logger.info(f"Content analysis pipeline processed {len(processed_content)} content items")
    ...
    async def test_pipeline_step_execution(self, test_prompts_dir, test_content_files):
        ...
        article_generation_agent = await get_article_generation_agent(test_prompts_dir, "en")
        seo_optimization_agent = await get_seo_optimization_agent(test_prompts_dir, "en")
        internal_docs_agent = await get_internal_docs_agent(test_prompts_dir, "en")
        content_formatting_agent = await get_content_formatting_agent(test_prompts_dir, "en")
        ...
        for content_context in content_models:
            try:
                ...
                assert result is not None, f"Pipeline result is None for {content_context.__class__.__name__}"
                assert isinstance(result, dict), f"Pipeline result should be dict, got {type(result)}"
                
                logger.info(f"Successfully processed {content_context.__class__.__name__} through pipeline")
                
            except Exception as e:
                logger.error(f"Pipeline execution failed for {content_context.__class__.__name__}: {e}")
    ...
    async def test_content_type_detection(self, test_content_files):
        ...
        content_items = await file_source.fetch_content(limit=10)
        ...
        for item in content_items:
            ...
            assert content_type in ["blog_post", "transcript", "other"], f"Unexpected content type: {content_type}"
        
        logger.info(f"Content type detection test passed for {len(content_items)} items")
    ...
    def test_pipeline_configuration_loading(self, test_pipeline_config):
        ...
        assert "pipelines" in test_pipeline_config, "Pipeline config missing 'pipelines'"
        assert "default" in test_pipeline_config["pipelines"], "Pipeline config missing 'default' pipeline"
        
        # Verify default pipeline steps
        default_pipeline = test_pipeline_config["pipelines"]["default"]
        expected_steps = [
            "AnalyzeContent", "ExtractSEOKeywords", "GenerateMarketingBrief",
            "GenerateArticle", "OptimizeSEO", "SuggestInternalDocs",
            "FormatContent", "ApplyDesignKit"
        ]
        ...
        assert "context_passing" in test_pipeline_config, "Pipeline config missing 'context_passing'"
        
        # Verify content sources configuration
        assert "content_sources" in test_pipeline_config, "Pipeline config missing 'content_sources'"
        assert test_pipeline_config["content_sources"]["enabled"], "Content sources not enabled"
        
        logger.info("Pipeline configuration loading test passed")

@pytest.mark.asyncio
async def test_complete_e2e_workflow(test_prompts_dir, test_content_files, test_pipeline_config):
    """
    ...
    assert "content_pipeline_agent" in result, "Missing content_pipeline_agent in result"
    ...
    processed_content = result["processed_content"]
    assert len(processed_content) > 0, "No content was processed in E2E workflow"
    ...
    logger.info(f"- Pipeline agent: {result['content_pipeline_agent'].__class__.__name__}")

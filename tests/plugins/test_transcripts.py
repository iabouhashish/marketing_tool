...
class TestExtractTranscriptMetadata:
    ...
    @patch('marketing_project.plugins.transcripts.tasks.parse_transcript')
    def test_extract_transcript_metadata_with_parsed_data(self, mock_parse, sample_transcript):
    ...
    @patch('marketing_project.plugins.transcripts.tasks.parse_transcript')
    def test_extract_transcript_metadata_no_timestamps(self, mock_parse, sample_transcript):
...
class TestEnhanceTranscriptWithOCR:
    ...
    def test_enhance_transcript_with_ocr_no_images(self, mock_enhance, mock_extract, sample_transcript):
    ...
    @patch('marketing_project.plugins.transcripts.tasks.enhance_content_with_ocr')
    def test_enhance_transcript_with_ocr_with_images(self, mock_enhance, sample_transcript):
        """Test enhancing transcript with OCR when images are provided."""
        image_urls = ['https://example.com/slide1.jpg', 'https://example.com/slide2.jpg']
        mock_enhance.return_value = {
...
class TestRouteTranscriptProcessing:
    ...
    def test_route_podcast_transcript(self, sample_app_context_transcript, sample_available_agents):
        """Test routing podcast transcript to appropriate agent."""
        sample_app_context_transcript.content.transcript_type = "podcast"
        
        result = route_transcript_processing(sample_app_context_transcript, sample_available_agents)
        
        assert "Successfully routed podcast transcript to transcripts_agent" in result
    
    def test_route_video_transcript(self, sample_app_context_transcript, sample_available_agents):
    ...
    def test_route_meeting_transcript(self, sample_app_context_transcript, sample_available_agents):
    ...
    def test_route_interview_transcript(self, sample_app_context_transcript, sample_available_agents):
    ...
    def test_route_general_transcript(self, sample_app_context_transcript, sample_available_agents):
    ...
    def test_route_transcript_no_agent_available(self, sample_app_context_transcript):
        ...
        result = route_transcript_processing(sample_app_context_transcript, available_agents)
        
        assert "No specialized agent for podcast transcript, using general processing" in result
    
    def test_route_transcript_wrong_content_type(self, sample_app_context_blog, sample_available_agents):
        """Test routing with wrong content type."""
        result = route_transcript_processing(sample_app_context_blog, sample_available_agents)
...
class TestIntegration:
    ...
    @patch('marketing_project.plugins.transcripts.tasks.extract_images_from_content')
    @patch('marketing_project.plugins.transcripts.tasks.enhance_content_with_ocr')
    def test_full_transcript_processing_workflow(self, mock_enhance, mock_extract, mock_parse, 
                                                sample_transcript, sample_available_agents):
        ...
        routing_result = route_transcript_processing(app_context, sample_available_agents)
...
class TestEdgeCases:
    ...
    def test_extract_metadata_with_minimal_transcript(self):
        ...
        with patch('marketing_project.plugins.transcripts.tasks.parse_transcript') as mock_parse:
            mock_parse.return_value = {
                'speakers': [],

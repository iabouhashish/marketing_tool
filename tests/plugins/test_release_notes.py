...
class TestExtractReleaseMetadata:
    ...
    def test_extract_release_metadata(self, mock_parse, sample_release_notes):
        ...
        assert result["breaking_changes_count"] == len(
            sample_release_notes.breaking_changes
        )
    ...
    @patch("marketing_project.plugins.release_notes.tasks.parse_release_notes")
    def test_extract_release_metadata_with_parsed_data(
        self, mock_parse, sample_release_notes
    ):
...
class TestEnhanceReleaseNotesWithOCR:
    ...
    def test_enhance_release_notes_with_ocr_no_images(
        self, mock_enhance, mock_extract, sample_release_notes
    ):
    ...
    @patch("marketing_project.plugins.release_notes.tasks.enhance_content_with_ocr")
    def test_enhance_release_notes_with_ocr_with_images(
        self, mock_enhance, sample_release_notes
    ):
        ...
        mock_enhance.assert_called_once_with(
            sample_release_notes.content, "release_notes", image_urls=image_urls
        )
...
class TestRouteReleaseProcessing:
    ...
    def test_route_major_release(
        ...
        result = route_release_processing(
    ...
    def test_route_general_release(
        self, sample_app_context_release, sample_available_agents
    ):
    ...
    def test_route_release_no_agent_available(self, sample_release_notes):
        ...
        assert (
            "No specialized agent for major release, using general processing" in result
        )

    def test_route_release_wrong_content_type(
...
class TestIntegration:
    ...
    @patch("marketing_project.plugins.release_notes.tasks.extract_images_from_content")
    @patch("marketing_project.plugins.release_notes.tasks.enhance_content_with_ocr")
    def test_full_release_notes_processing_workflow(
        self,
        mock_enhance,
        mock_extract,
        mock_parse,
...
class TestEdgeCases:
    ...
    def test_extract_metadata_with_minimal_release_notes(self):
        ...
        with patch(
            "marketing_project.plugins.release_notes.tasks.parse_release_notes"
        ) as mock_parse:
            mock_parse.return_value = {
                "version": "1.0.0",
                "release_date": None,
